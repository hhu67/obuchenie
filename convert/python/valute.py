...
def get_supported_codes(api_key):
    """Получает список всех доступных валют и стран."""
    url = f"https://v6.exchangerate-api.com/v6/{api_key}/codes"
    try:
        response = requests.get(url)
        data = response.json()
        if data['result'] == 'success':
            return data['supported_codes'] # Формат: [['USD', 'United States Dollar'], ...]
    except:
        return []
    return []

def find_suggestions(user_input, codes_list):
    """Ищет валюты, начинающиеся на те же 2 буквы."""
    prefix = user_input[:2].upper()
    # Фильтруем список: ищем коды, которые начинаются на prefix
        ...
        while True:
            ...
            if code in valid_codes:
                ...
                if suggestions:
                    ...
                    print("К сожалению, подходящих вариантов не найдено.")
                print("-" * 20)
